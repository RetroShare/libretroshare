...
#if defined(RS_JEREMY_RIFKIN_CPPTRACE)
...
{
	cpptrace::generate_trace(1, maxFrames).print();
}
...
/* Freely inspired by Timo Bingmann implementation
 * https://panthema.net/2008/0901-stacktrace-demangled/
 * https://web.archive.org/web/20230815072644/https://panthema.net/2008/0901-stacktrace-demangled/
 */
void print_stacktrace(
        bool demangle, FILE* out, unsigned int maxFrames )
{
	...
	/* Skip first address which is print_stacktrace return address itself */
	auto callerRetPtr = addrlist + 1;
	...
	if(!demangle)
	{
		...
		backtrace_symbols_fd(callerRetPtr, addrlen, outFd);
		return;
	}

	// resolve addresses into strings containing "filename(function+address)",
	// this array must be free()-ed
	char** symbollist = backtrace_symbols(callerRetPtr, addrlen);
...
/* Freely inspired by the solution proposed by Louis Semprini on this thread
 * https://stackoverflow.com/questions/8115192/android-ndk-getting-the-backtrace/35586148
 * https://web.archive.org/web/20240121102501/https://stackoverflow.com/questions/8115192/android-ndk-getting-the-backtrace/35586148
 */
 ...
 * https://web.archive.org/web/20221005052001/https://oroboro.com/stack-trace-on-crash/
 */
...
[[ noreturn ]]
/*static*/ void CrashStackTrace::abortHandler(int signum)
{
...
#ifndef __ANDROID__
	/** Notify the user which signal was caught. We use fprintf, because this
	 * is the most basic output function. Once you get a crash, it is
	 * possible that more complex output systems like streams and the like
	 * may be corrupted. So we make the most basic call possible
	 * to the lowest level, most standard print function. */
